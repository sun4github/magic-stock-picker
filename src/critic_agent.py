...
critic_agent = LlmAgent(
    ...
    instruction=(
        ...
        "REPORT_VINTAGE below says when the report was written; VERIFIED_FIGURES were "
        "computed TODAY. That gap is deliberate — a reader is deciding today, so the "
        "critique should be against today's facts — but it creates one specific way to "
        "be wrong, and it is the easiest finding in this whole review to raise by "
        "mistake:\n"
        "- **A market-derived figure that has merely MOVED is not an error.** Share "
        "price, market capitalisation, enterprise value, and every multiple computed "
        "from them (price-to-earnings, earnings yield, PEG) change every session. If "
        "the report states one that was right when it was written, that is not a "
        "contradiction of VERIFIED_FIGURES and must NOT be raised as a factual error, "
        "however large the gap. Filing-derived figures — debt, cash, equity, assets, "
        "operating profit — do not move between filings, so a mismatch there IS a "
        "finding.\n"
        "- **What to do instead.** If the valuation has moved far enough to change the "
        "argument, say exactly that, as its own point: the report reasoned at the price "
        "of its date, the stock is at a different price today, and here is the part of "
        "the thesis that no longer follows. That is a useful finding about the "
        "reasoning. 'The report says the market cap is X but it is Y' on its own is "
        "not — it is a clock.\n"
        "- **Never ask for a price to be restated to today's.** The revised report is "
        "re-stamped with a fresh price section by the pipeline itself; a reviser "
        "rewriting prices inside the prose would produce a document whose numbers came "
        "from two different days with nothing to say which was which.\n\n"
        + _CRITIC_CONTEXT_BLOCKS
    ),
...
reviser_agent = LlmAgent(
    ...
    instruction=(
        ...
        "## Do not re-price the report\n"
        "REPORT_VINTAGE says when the report you are revising was written, and the "
        "figures beside you were computed today. Leave every share price, market "
        "capitalisation and derived multiple in the prose AS IT WAS unless the critic "
        "found it wrong ON ITS OWN DATE. A stored report carries a deterministic price "
        "section, stamped with its own timestamp, that the pipeline regenerates around "
        "your text — so quietly updating prices inside the prose produces a document "
        "whose figures come from two different days with nothing to say which is which. "
        "If the critic's point is that the valuation has MOVED, address that as a "
        "change in the argument (what the move means for the thesis), not by editing "
        "the old numbers to new ones.\n\n"
        "<REPORT_VINTAGE>\n{report_vintage}\n</REPORT_VINTAGE>\n\n"
